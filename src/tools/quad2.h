...
public:
	...
	quad2(float x, float y, float w, float h)
	    : quad2(glm::vec2(x, y), glm::vec2(w, h)){};
	...
	quad2() : quad2(0, 0, 0, 0){};
	glm::vec2 v00() const { return glm::vec2(x, y); }
	glm::vec2 v01() const { return glm::vec2(x, y + h); }
	glm::vec2 v10() const { return glm::vec2(x + w, y); }
	glm::vec2 v11() const { return glm::vec2(x + w, y + h); }
	...
	quad2 operator>>(const quad2 &c) const {
		return operator>>(c.v00()) >> c.v11();
	}
	quad2 reflectX() const { return quad2(x + w, y, -w, h); }
	quad2 reflectY() const { return quad2(x, y + h, w, -h); }
	quad2 operator*(float a) const { return quad2(pos, size * a); }
	quad2 operator/(float a) const { return operator*(1.0f / a); }
	quad2 operator+(const glm::vec2 &c) const { return quad2(pos + c, size); }
	quad2 operator-(const glm::vec2 &c) const { return operator+(-c); }
	bool contains(const glm::vec2 &c) const {
