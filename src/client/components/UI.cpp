...
class UI *pUI;
UI *g_UI() { return pUI != nullptr ? pUI : new UI(); }
...
void UI::Tick() {
	...
	if (CursorEnabled()) {
		bool buttonState = g_Input()->mouse[SDL_BUTTON_LEFT] != 0;
		...
		if (target != nullptr && buttonState && !prevButtonState) {
			target->OnMouseDown(cursorPosition);
			clickedTarget = target;
			prevCursorPosition = cursorPosition;
		} else if (target != nullptr && prevButtonState && !buttonState) {
		...
		if (clickedTarget != nullptr && buttonState &&
...
bool UI::CursorEnabled() const { return SDL_GetRelativeMouseMode() == 0u; }
